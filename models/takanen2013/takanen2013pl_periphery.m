function periphOutput = takanen2013pl_periphery(insig,fs,outputPlot)
...
%     
%
%   Url: http://amtoolbox.org/amt-1.6.0/doc/modelstages/takanen2013_periphery.php
...
%PL
spl = 70; % sound pressure level of the input signal
...
if isstruct(insig)
    ...
    [V,Y,E,CF] = verhulst2012rd(insig,fs,'all',[spl spl]);
    for ifc = 1:length(fc)
        %idx_CF(ifc) = find(CF >= fc(ifc),1,'first');
        idx_CF(ifc) = find(CF <= fc(ifc),1,'first');
    end
    CF = CF(idx_CF);
    V = V(:,idx_CF,:);
...
%PL
periphOutput.cochlear = cochlear;
