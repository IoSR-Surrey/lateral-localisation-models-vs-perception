function out = bootstrap_intercept_ci(est_angle, listexp_data, opts)
%BOOTSTRAP_INTERCEPT_CI Bootstrap confidence interval for regression intercept.
%
%   out = bootstrap_intercept_ci(est_angle, listexp_data, opts)
%
% Quantifies uncertainty of the OLS intercept beta0 in the linear fit
...
%
% Output struct fields: intercept, intercept_ci ([lo hi]), intercept_se,
% intercept_boot, n_boot, ci_level, unit.
...
if numel(est) ~= numel(resp0)
    error('bootstrap_intercept_ci:size', ...
...
switch unit
    case 'nested'
        ...
        for b = 1:n_boot
            ...
            interceptb(b) = local_intercept(resp_b, est);
        end
    ...
    case 'condition'
        ...
        for b = 1:n_boot
            idx = randi(n, n, 1);
            interceptb(b) = local_intercept(resp0(idx), est(idx));
        end

    otherwise
        error('bootstrap_intercept_ci:unit', 'Unknown resample unit: %s', unit);
end

alpha = 1 - opts.ci_level;
lo = quantile(interceptb, alpha / 2);
hi = quantile(interceptb, 1 - alpha / 2);

out = struct();
out.intercept = local_intercept(resp0, est);
out.intercept_ci = [lo, hi];
out.intercept_se = std(interceptb, 'omitnan');
out.intercept_boot = interceptb;
out.n_boot = n_boot;
out.ci_level = opts.ci_level;
out.unit = unit;
end

function intercept = local_intercept(x, y)
% OLS intercept of y ~ x (perceived = x, estimated = y).
ok = ~isnan(x) & ~isnan(y);
if nnz(ok) < 3
    intercept = NaN;
    return
...
cxy = cov(xv, yv, 1);
slope = cxy(1, 2) / vx;
intercept = mean(yv) - slope * mean(xv);
end
